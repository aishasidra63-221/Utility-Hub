...
export default function FaceBlur() {
  useSEO({
    ...
    description:
      "Automatically detect and blur all faces in any image. AI-powered, 100% private — runs in your browser, nothing uploaded.",
  });
  ...
  const autoDetectFaces = useCallback(async (img: HTMLImageElement, _file: File, intensity: number) => {
    try {
      ...
      if (!_detectionPipe) {
        setProgress("Downloading face detection model (~10MB, first time only)…");
        _detectionPipe = await pipeline(
          ...
          "Xenova/yolov9-c-sharpenss-face-detection",
          {
            ...
            progress_callback: (prog: any) => {
              if ((prog.status === "progress" || prog.status === "download") && prog.total > 0) {
                setProgress(`Downloading model: ${Math.round((prog.loaded / prog.total) * 100)}%`);
              }
            },
          }
        ).catch(() => null);
      ...
      let detectedRects: FaceRect[] = [];

      if (_detectionPipe) {
        setProgress("Detecting faces…");
        const imageUrl = URL.createObjectURL(_file ?? new Blob());
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const detections: any[] = await (_detectionPipe as any)(imageUrl, { threshold: 0.4 });
        URL.revokeObjectURL(imageUrl);

        detectedRects = detections
          .filter((d: any) => d.label === "face" || d.score > 0.4)
          .map((d: any) => ({
      ...
      if (detectedRects.length === 0) {
        // Fallback: simple Viola-Jones-like skin-tone detection heuristic
        detectedRects = detectFacesFallback(img);
      }

      setFaces(detectedRects);
      if (detectedRects.length > 0) {
        await applyBlurToFaces(img, detectedRects, intensity);
      } else {
        setError("No faces detected. Switch to manual mode to draw blur areas.");
        setLoading(false);
        setProgress("");
      }
    } catch {
      _detectionPipe = null;
      // Fallback
      const detectedRects = detectFacesFallback(img);
      setFaces(detectedRects);
      if (detectedRects.length > 0) {
        await applyBlurToFaces(img, detectedRects, intensity);
      } else {
  ...
  function detectFacesFallback(img: HTMLImageElement): FaceRect[] {
    ...
    for (let by = 0; by < height - blockSize; by += blockSize) {
      for (let bx = 0; bx < width - blockSize; bx += blockSize) {
        ...
        if (skinCnt > blockSize * blockSize * 0.55) {
          skinBlocks.push({ bx, by });
        }
      }
    }
  ...
  const applyBlurToFaces = useCallback(async (img: HTMLImageElement, faceRects: FaceRect[], intensity: number) => {
    ...
    for (const face of faceRects) {
      const padding = Math.round(Math.min(face.w, face.h) * 0.08);
      const padded: FaceRect = {
        x: Math.max(0, face.x - padding),
        y: Math.max(0, face.y - padding),
        w: Math.min(canvas.width - face.x + padding, face.w + padding * 2),
        h: Math.min(canvas.height - face.y + padding, face.h + padding * 2),
      };
      applyBoxBlur(imgData.data, canvas.width, canvas.height, padded, intensity, 4);
    }
  ...
  // Re-draw overlay when faces change
  useCallback(() => drawOverlay(), [drawOverlay])();
  ...
  const download = () => {
    ...
    a.href = result;
    a.download = fileName || "face_blurred.jpg";
    a.click();
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="flex flex-col items-center text-center gap-3">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Automatically detect and blur all faces in any photo — browser only, no uploads, privacy first.
            </p>
          </div>
      ...
      <div className="mb-5 flex items-start gap-2.5 bg-primary/8 border border-primary/20 rounded-xl px-4 py-3 text-sm text-primary">
        ...
        <span>
          <strong>Privacy focused.</strong> Everything runs in your browser. No face data, no image, no anything is sent to any server.
        </span>
      </div>
      ...
      {!original ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            <div>
              ...
              <div className="flex gap-2">
                ...
                >
                  <MousePointer className="w-3.5 h-3.5" /> Manual
                </button>
              </div>
            </div>
            <div>
              <p className="text-sm font-medium text-muted-foreground mb-2">Blur intensity: <span className="text-foreground font-semibold">{blurIntensity}</span></p>
              <input
                type="range" min={4} max={40} value={blurIntensity}
                onChange={(e) => setBlurIntensity(Number(e.target.value))}
                className="w-full accent-primary"
              />
              <div className="flex justify-between text-xs text-muted-foreground mt-0.5"><span>Subtle</span><span>Strong</span></div>
            </div>
          </div>
          <ImageDropZone
            dragOver={dragOver}
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <div className="px-4 py-2.5 border-b border-border flex items-center justify-between">
                ...
                <span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold">✓ {faces.length} face{faces.length !== 1 ? "s" : ""}</span>
              </div>
              <div className="p-3 bg-muted/20 flex items-center justify-center min-h-[180px]">
          ...
          <div className="bg-card border border-border rounded-xl px-4 py-3 text-sm text-muted-foreground">
            💡 <strong>Tip:</strong> For missed faces, switch to manual mode and draw over any remaining areas.
          </div>
        </div>
      ) : (
        <div className="space-y-5">
            ...
            <>
              {faces.length > 0 && (
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-2.5 text-sm text-emerald-700 dark:text-emerald-400">
                  {faces.length} area{faces.length !== 1 ? "s" : ""} marked. Click Remove to apply blur.
                </div>
              )}
              {!faces.length && (
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-2.5 text-sm text-amber-700 dark:text-amber-400">
                  Draw rectangles over faces on the image below.
                </div>
              )}
              <div className="relative bg-card border border-border rounded-xl overflow-hidden">
                <div className="px-4 py-2.5 border-b border-border flex items-center justify-between">
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Draw over areas to blur</p>
                  {faces.length > 0 && <button onClick={() => setFaces([])} className="text-xs text-destructive hover:underline">Clear all</button>}
                </div>
                <div className="relative">
                  <img src={original} alt="original" className="w-full object-contain block" style={{ maxHeight: 420 }} />
                  <canvas
                    ref={overlayRef}
